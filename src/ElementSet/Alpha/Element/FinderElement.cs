...
namespace Spring2.DataTierGenerator.Element {
	...
	public static void ParseFromXml(XmlNode node, IList finderElements) {
		...
		foreach (XmlNode finderNode in node.ChildNodes) {
			...
			PropertyElement.ParseFromXml(GetChildNodeByName(finderNode, PROPERTIES), finderElement.Properties);
	...
	public static ArrayList ParseFromXml(XmlNode root, EntityElement entity, ParserValidationDelegate vd) {
		...
		foreach (XmlNode node in elements) {
			...
			if (field != null) {
			    ...
			    vd(ParserValidationArgs.NewError("property (" + field.Name + ") defined in finder method " + finder.Name + " was not defined as a property"));
			}
