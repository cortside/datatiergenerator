...
//		sb.Append(" sqlentity=\"").Append(entity.SqlEntity.Name).Append("\"");
//		sb.Append(">\n");
...
//			    field.IsViewColumn = objDataRow["IsViewColumn"].ToString() == "1";
	...
	private DataTable GetTableColumns(Entity entity, SqlConnection connection) {
	    ...
	    sql = sql + "  	INNER JOIN syscolumns ON INFORMATION_SCHEMA.COLUMNS.COLUMN_NAME = syscolumns.name  AND syscolumns.id = OBJECT_ID('" + entity.SqlEntity.Name + "') ";
	    sql = sql + "	left join syscolumns vc on INFORMATION_SCHEMA.COLUMNS.COLUMN_NAME = vc.name AND vc.id = OBJECT_ID('" + entity.SqlEntity.Name + "') ";
	    sql = sql + "  	WHERE INFORMATION_SCHEMA.COLUMNS.TABLE_NAME = '" + entity.SqlEntity.Name + "' ";
	    ...
	    if (options.UseViews && entity.SqlEntity.View.Length>1) {
		sql = sql + "union ";
		...
		sql = sql + " 	WHERE INFORMATION_SCHEMA.COLUMNS.TABLE_NAME = '" + entity.SqlEntity.View + "' AND syscolumns.id = OBJECT_ID('" + entity.SqlEntity.View + "') ";
		sql = sql + " 	and INFORMATION_SCHEMA.COLUMNS.COLUMN_NAME not in (select INFORMATION_SCHEMA.COLUMNS.COLUMN_NAME from INFORMATION_SCHEMA.COLUMNS where INFORMATION_SCHEMA.COLUMNS.TABLE_NAME = '" + entity.SqlEntity + "') ";
	    }
