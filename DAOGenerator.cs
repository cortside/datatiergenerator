        ...
        private void CreateInsertMethod(StringBuilder sb) {
            ...
            for (intIndex = 0; intIndex < fields.Count; intIndex++) {
                ...
                if (objField.IsIdentity) {
                    if (options.UseDataTypes) {
                        sb.Append("\t\tpublic IdType Insert(");
                    } else {
                    ...
                    if (options.UseDataTypes) {
                        sb.Append("\t\tpublic StringType Insert(");
                    } else {
...
//            sb.Append("\t\t\ttry {\n");
            ...
            for (intIndex = 0; intIndex < fields.Count; intIndex++) {
                ...
                if (!objField.IsViewColumn) {
					if (objField.IsIdentity || objField.IsRowGuidCol) {
						...
						sb.Append("\t\t\t\t" + objField.CreateSqlParameter(false, true, options.UseDataTypes));
					}
                }
                objField = null;
            }
            ...
            for (intIndex = 0; intIndex < fields.Count; intIndex++) {
                ...
                if (objField.IsIdentity || objField.IsRowGuidCol) {
                    ...
                    if (objField.IsIdentity)
                        //sb.Append("\t\t\t\treturn Int32.Parse(cmd.Parameters[\"@" + objField.ColumnName.Replace(" ", "_") + "\"].Value.ToString());\n");
						sb.Append("\t\t\t\treturn Int32.Parse(cmd.Parameters[\"RETURN_VALUE\"].Value.ToString());\n");
                    else
                        sb.Append("\t\t\t\treturn Guid.NewGuid(cmd.Parameters[\"@" + objField.ColumnName.Replace(" ", "_") + "\"].Value.ToString());\n");
                }
                objField = null;
            }
        ...
        private void CreateDeleteMethods(StringBuilder sb) {
            ...
            for (intIndex = 0; intIndex < fields.Count; intIndex++) {
                ...
                if (objField.IsIdentity || (options.GenerateProcsForForeignKey && (objField.IsRowGuidCol || objField.IsPrimaryKey || objField.IsForeignKey))) {
                    ...
                    // Append the method header
                    sb.Append("\t\t/// <summary>\n");
                    sb.Append("\t\t/// Deletes a record from the " + entity.Name + " table by " + objField.ColumnName + ".\n");
                    sb.Append("\t\t/// </summary>\n");
                    sb.Append("\t\t/// <param name=\"\"></param>\n");
...
//                    sb.Append("\t\t\ttry {\n");
                    ...
                    sb.Append("\t\t\t\t" + objField.CreateSqlParameter(false, false, options.UseDataTypes));
                    sb.Append("\n");
...
//            sb.Append("\t\t\tSqlConnection	objConnection;\n");
            ...
            for (intIndex = 0; intIndex < fields.Count; intIndex++) {
                ...
                if (objField.IsIdentity || objField.IsRowGuidCol || objField.IsPrimaryKey || objField.IsForeignKey) {
                    ...
                    // Append the method header
                    sb.Append("\t\t/// <summary>\n");
                    sb.Append("\t\t/// Selects a record from the " + entity.Name + " table by " + objField.ColumnName + ".\n");
                    sb.Append("\t\t/// </summary>\n");
                    sb.Append("\t\t/// <param name=\"\"></param>\n");
				...
				if (options.UseDataTypes) {
					...
					switch (objField.SqlType.ToLower()) {
						case "bytes":
							...
							break;
						default: 
							sb.Append("\t\t\tdata.").Append(objField.ColumnName).Append(" = ").Append("dataReader.Get").Append(objField.ReaderType).Append("(").Append("dataReader.GetOrdinal(\"").Append(objField.ColumnName).Append("\")").Append(");\n");
							break;
					}
				}
				objField = null;
